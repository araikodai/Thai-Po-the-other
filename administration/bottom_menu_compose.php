...
function createNewElement( $type, $source)
{
    ...
    if( $source ) {
        ...
        if( !$sourceActive ) {
            ...
            //create from source
            db_res( "INSERT INTO `" . $oMenu->sDbTable . "`(`Name`, `Caption`, `Icon`, `Link`, `Script`, `Target`)
                    SELECT `Name`, `Caption`, `Icon`, `Link`, `Script`, `Target` FROM `" . $oMenu->sDbTable . "` WHERE `ID`=$source" );
            $newID = db_last_id();
        }
    } else {
        //create new
