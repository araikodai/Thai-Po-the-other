...
class BxFilesPageAlbumsMy extends BxDolPageView
{
    ...
    function getBlockCode_add ()
    {
        ...
        $aPrivFieldView = $oAlbumPrivacy->getGroupChooser($this->iOwnerId, $this->oConfig->getUri(), 'album_view', array(), _t('_' . $this->oConfig->getMainPrefix() . '_album_view'));
    ...
    function getBlockCode_addObjects ($iBoxId)
    {
        ...
        if (!$this->oSearch->oModule->isAllowedAdd()) {
            ...
            $oUploader->sWorkingFile = $oUploader->sMultiUploaderParams['accept_file'] = $sLink;
        ...
        if($this->oSearch->aCurrent['paginate']['totalNum'] > 0) {
            ...
            $aUnit = array(
                'bx_if:show_class' => array(
                    'condition' => true,
                    'content' => array(
                        'content' => 'bx-def-bc-margin'
                    )
                ),
                'main_code_id' => $sAreaId,
                'main_code' => $sCode,
                'paginate' => '',
                'manage' => $this->oSearch->showAdminActionsPanel('', $aBtns, 'entry', false, false),
                'bx_if:hidden' => array(
                    'condition' => false,
                    'content' => array()
                )
