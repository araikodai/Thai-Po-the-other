...
require_once('BxVideosPrivacy.php');
require_once('BxVideosSearch.php');

class BxVideosPageAlbumsMy extends BxDolPageView
{
    var $oTemplate;
    var $oConfig;
    var $oDb;
    ...
    function BxVideosPageAlbumsMy (&$oShared, $iOwnerId, $aParams = array())
    {
        ...
        $this->oSearch = new BxVideosSearch('album', $this->aAddParams[1], 'owner', getUsername($this->iOwnerId));
        ...
        $sCaption = str_replace('{nickname}', getUsername($this->iOwnerId), $this->oConfig->getGlParam('profile_album_name'));
        ...
        if (!is_array($aOwnerDefAlbumInfo) || empty($aOwnerDefAlbumInfo)) {
            $aData = array(
                'caption' => $sCaption,
                'location' => _t('_' . $this->oConfig->getMainPrefix() . '_undefined'),
                'owner' => $this->iOwnerId,
    ...
    function getBlockCode_addObjects ($iBoxId)
    {
        ...
        if (!$this->oSearch->oModule->isAllowedAdd()) {
            ...
            $sCode = $this->oTemplate->parseHtmlByName('default_margin.html', array('content' => $oUploader->GenMainAddVideoForm(array('album' => $this->aAddParams[1]))));
        }
    ...
    function getTopMenu ($sMode = 'main')
    {
        ...
        foreach ($this->aSystemBlocks as $sKey => $aValue) {
            ...
            if ($aValue['level'] == $iCheck)
                $aTopMenu[_t('_' . $this->oConfig->getMainPrefix() . '_album_' . $sKey)] = array(
