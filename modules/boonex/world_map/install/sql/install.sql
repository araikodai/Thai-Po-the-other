...
SET @iMaxOrderProfile = (SELECT `Order` + 1 FROM `sys_page_compose` WHERE `Page` = 'profile' AND `Column` = 1 ORDER BY `Order` DESC LIMIT 1);
...
('profile', '1140px', 'Location', '_Location', 1, @iMaxOrderProfile, 'PHP', 'return BxDolService::call(''wmap'', ''location_block'', array(''profiles'', $this->oProfileGen->_iProfileID));', 1, 28.1, 'non,memb', 0);
