...
class BxWmapModule extends BxDolModule
{
    ...
    function serviceEditLocation ($sPart, $iEntryId)
    {
        ...
        $aVars = array (
            'info' => sprintf(_t('_bx_wmap_edit'), "<a href=\"{$sBackLink}\">{$aLocation['title']}</a>", _t($this->_aParts[$sPart]['title_singular'])),
            'map' => $sMap,
        );
    ...
    function _saveData ($iEntryId, $sPart, $iZoom, $sMapType, $fLat, $fLng, $sMapClassInstanceName = '', $sAddress = 'null', $sCountry = 'null')
    {
        ...
        $sAddress = $sAddress && 'null' != $sAddress ? process_db_input($sAddress, BX_TAGS_STRIP) : ($aLocation ? process_db_input($aLocation['address'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION) : '');
        $sCountry = $sCountry && 'null' != $sCountry ? process_db_input($sCountry, BX_TAGS_STRIP) : ($aLocation ? process_db_input($aLocation['country'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION) : '');
