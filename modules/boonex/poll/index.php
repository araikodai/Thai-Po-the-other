        ...
        switch($aPollSettings['action']) {
            ...
            case 'poll_home' :
                ...
                if($aPollSettings['action'] == 'show_poll_info' && $aPollInfo) {
                    ...
                    $sPageCaption = _t('_bx_poll_view', $aPollInfo[0]['poll_question']);
                    $_page['header']        = $sPageCaption ;
                    $_page['header_text']   = $sPageCaption ;

                    $oPoll -> _oTemplate -> addJsTranslation(array('_are you sure?'));
                    $oPoll -> _oTemplate -> setPageDescription($aPollInfo[0]['poll_question']);
                    $oPoll -> _oTemplate -> addPageKeywords($aPollInfo[0]['poll_answers'], BX_POLL_ANS_DIVIDER);
                ...
                if($aPollSettings['action'] == 'show_poll_info')
                    $_page_cont[$iIndex]['page_main_code'] = !$aPollInfo ? MsgBox(_t('_Empty')) : $sInitPart . $oViewPoll -> getCode();
                else
