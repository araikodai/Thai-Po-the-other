...
class BxGroupsSearchResult extends BxDolTwigSearchResult
{
    var $aCurrent = array(
        ...
        'ownFields' => array('id', 'title', 'uri', 'created', 'author_id', 'thumb', 'rate', 'fans_count', 'country', 'city', 'desc'),
    ...
    function getAlterOrder()
    {
        if ($this->aCurrent['sorting'] == 'last') {
            ...
            $aSql = array();
            $aSql['order'] = " ORDER BY `bx_groups_main`.`fans_count` DESC, `bx_groups_main`.`views` DESC";
            return $aSql;
        }
        return array();
    }
