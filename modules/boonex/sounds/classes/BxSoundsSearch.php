...
class BxSoundsSearch extends BxTemplSearchResultSharedMedia
{
    ...
    function getImgPath ($iId, $sImgType = 'browse')
    {
        $iId = (int)$iId;
        switch ($sImgType) {
            case 'file':
                $sImgUrl = $this->aConstants['filesDir'] . $iId . '.mp3';
                break;

            default:
                $sImgUrl = $this->aConstants['filesDir'] . $iId . $this->aConstants['picPostfix'];
                if(!file_exists($sImgUrl))
                    $sImgUrl = $this->aConstants['filesDir'] . 'default.png';
                break;
        }
        return $sImgUrl;
    }
