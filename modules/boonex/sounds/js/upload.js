...
BxSoundUpload.prototype._loading = function (bShow) {
    ...
    var oLoadingAjax = $('.upload-loading-container .loading_ajax');
    if(bShow) {
        oLoadingAjax.css('left', (oLoading.parent().width() - oLoading.width())/2);
        oLoadingAjax.css('top', (oLoading.parent().height() - oLoading.height())/2);
        oLoading.show();
    }
    else
        oLoading.hide();
