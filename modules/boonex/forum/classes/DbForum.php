...
class DbForum extends BxDb
{
    ...
    function searchMessages ($s, $u, $f, $type, $posts, $start, &$num = null)
    {
        ...
        if (strlen($s) > 2) {
            if ($gConf['fulltext_search']) {
                ...
                $match = '';
                if ('msgs' == $type)
                    $match = "MATCH (`post_text`) AGAINST ('$s')";
                else // titles
                    $match = "MATCH (`topic_title`) AGAINST ('$s')";
    ...
    function getRecentTopics ($start)
    {
        ...
        return $this->getAll ( "SELECT
                ...
                `tf`.`forum_id`, `tf`.`forum_uri`, `tf`.`forum_title`,
    ...
    {
        $sql = "INSERT INTO " . TF_FORUM_ACTIONS_LOG . " SET `user_name` = '$u', `id` = '$id', `action_name` = '" . $action . "', `action_when` = UNIX_TIMESTAMP()";
        return $this->query($sql);
    }
