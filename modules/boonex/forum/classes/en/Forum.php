...
class Forum extends ThingPage
{
     ...
     */
    function getSearchResultsXML ($text, $type, $forum, $u, $disp, $start = 0)
    {
        global $gConf;
...
EOS;
    ...
    function getSearchXML ($wp = false)
    {
        ...
        $s = $this->getCategsShortXML();

        $cu = $this->getUrlsXml ();

        if ($wp) {
            $this->setTitle ('<![CDATA[Search The Forum]]>');
...
EOF;
    ...
    function getPostsXML ($topic_uri, $wp)
    {
        ...
        while ( list (,$r) = each ($a) ) {
            ...
            encode_post_text ($r['post_text'], $wp, 1, true);
...
EOF;
    ...
    {
        return $this->getXxxTopicsXML ($wp, 'Hidden Topics', '_Hidden topics', 'getHiddenTopics', $start);
    }
    ...
    {
        return $this->getXxxTopicsXML ($wp, 'My Topics', 'Topics you participate in', 'getMyThreadsTopics', $start);
    }
    ...
    {
        return $this->getXxxTopicsXML ($wp, 'Topics subscriptions', 'Topics you have subscribed for', 'getMyFlaggedTopics', $start);
    }
...
EOF;
    ...
    function getRecentTopicsXML ($wp, $start = 0)
    {
        ...
        while ( list (,$r) = each ($a) ) {
                ...
                encode_post_text ($r['topic_title'], $wp, 1);
                encode_post_text ($r['forum_title'], $wp, 1);
                encode_post_text ($r['cat_name'], $wp, 1);
...
EOF;
        ...
        if ($wp) {
            $this->setTitle('Recent Topics');
            $li = $this->_getLoginInfo ($user);
            return $this->addHeaderFooter ($li, "<topics><pages num=\"$num\" per_page=\"{$gConf['topics_per_page']}\">$p</pages>{$t}</topics>");
        } else {
            $cu = $this->getUrlsXml ();
            return "<root>$cu<topics><pages num=\"$num\" per_page=\"{$gConf['topics_per_page']}\">$p</pages>{$t}</topics></root>";
        }
    }
...
EOF;
    ...
    function topicAutoContinue ($t)
    {
        ...
        $sNewTopicSubj = ($aTopicFirst ? $aTopicFirst['topic_title'] : $t['topic_title']) . sprintf(' - part %d', $iTopicIndex);
        $sOldTopicUrl = $gConf['url']['base'] . sprintf($gConf['rewrite']['topic'], $t['topic_uri']);
        $sNewTopicText = sprintf('<div class="forum_robot_msg bx-def-font">&#8592; %s</div>', '<a href="' . $sOldTopicUrl . '">' . $t['topic_title'] . '</a>');
        ...
        $sOldTopicText = sprintf('<div class="forum_robot_msg bx-def-font">%s &#8594;</div>', '<a href="' . $sNewTopicUrl . '">' . $aNewTopic['topic_title'] . '</a>');
...
EOF;
    ...
    function deletePostXML ($post_id, $topic_id, $forum_id)
    {
        ...
        if ($this->_checkUserPerm ('', $f['forum_type'], 'del', $f['forum_id']))
            $no_access = false;
        if ($no_access && ($user = $this->fdb->getPostUser((int)$post_id)) == $this->_getLoginUserName())
...
EOF;
    ...
    function getRssUpdatedTopics ()
    {
        ...
        while ( list (,$r) = each ($a) ) {
            ...
            $td = orca_mb_replace('/#/', $r['count_posts'], '# posts') . ' &#183; ' . orca_mb_replace('/#/', $ui[$r['last_post_user']]['title'], 'last reply by #') . ' &#183; ' . $r['cat_name'] . ' &#187; ' . $r['forum_title'];
        ...
        return $this->_rssFeed ('Updated Topics', '', 'Updated Topics', $lastBuildDate, $items);
    }
    ...
    function getRssUser ($user, $sort)
    {
        ...
        while ( list (,$r) = each ($a) ) {
            ...
            $items .= $this->_rssItem ($r['topic_title'], sprintf($gConf['rewrite']['topic'], $r['topic_uri']), "{$r['user']}: {$td}", $r['when']);
        }
        ...
        $sTitle = sprintf("%s's forum posts", $aUser['title']);
    ...
    function getRssAll ($sort)
    {
        ...
        return $this->_rssFeed ('Forum Posts', '', 'Forum Posts', $lastBuildDate, $items);
    }
...
EOF;
    ...
    function _getUserInfoReadyArray ($user, $bWrapWithCdata = true)
    {
        ...
        return array ('avatar' => $aa['avatar'], 'url' => $aa['profile_url'], 'title' => $aa['profile_title'], 'onclick' => $aa['profile_onclick'], 'role' => $aa['role']);
    }
    ...
    function setTitle ($s)
    {
        ...
        $GLOBALS['gConf']['title'] = '<![CDATA[' . $s . ' :: Forum ' . ']]>';
    }
