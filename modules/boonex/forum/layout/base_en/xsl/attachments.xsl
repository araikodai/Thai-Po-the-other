        ...
        <div class="bx-def-margin-top">
            ...
            <xsl:if test="$files">
                <xsl:for-each select="$files/file">
                    ...
                    <xsl:if test="1 = @image">
                        <span class="bullet"> &#183; </span><a href="javascript:void(0);" onclick="f.insertImageToPost('tinyEditor_{../../post_id}', '{@hash}')">insert to post</a>
                    </xsl:if>
                </xsl:for-each>
            </xsl:if>
            <div class="forum_file_attachment"><input type="file" name="attachments[]" /></div>
            <a href="javascript:void(0);" onclick="jQuery('.forum_file_attachment:last').after(jQuery('.forum_file_attachment:first').clone())">Attach one more file</a>
        </div>
