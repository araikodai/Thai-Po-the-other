...
<xsl:template match="pages/p">
    ...
    <xsl:call-template name="paginate_number">
        ...
        <xsl:with-param name="link_first" select="concat($base_url, '?action=goto&amp;my_flags=1&amp;start=0')" />
        <xsl:with-param name="onclick_first">return document.f.showMyFlags ('0')</xsl:with-param>

        <xsl:with-param name="link_prev" select="concat($base_url, '?action=goto&amp;my_flags=1&amp;start=', $start_prev)" />
        <xsl:with-param name="onclick_prev">return document.f.showMyFlags ('<xsl:value-of select="$start_prev" />')</xsl:with-param>        

        <xsl:with-param name="link_next" select="concat($base_url, '?action=goto&amp;my_flags=1&amp;start=', $start_next)" />
        <xsl:with-param name="onclick_next">return document.f.showMyFlags ('<xsl:value-of select="$start_next" />')</xsl:with-param>

        <xsl:with-param name="link_last" select="concat($base_url, '?action=goto&amp;my_flags=1&amp;start=', $start_last)" />    
        <xsl:with-param name="onclick_last">return document.f.showMyFlags ('<xsl:value-of select="$start_last" />')</xsl:with-param>

        <xsl:with-param name="link_curr" select="concat($base_url, '?action=goto&amp;my_flags=1&amp;start=', @start)" />
        <xsl:with-param name="onclick_curr">return document.f.showMyFlags ('<xsl:value-of select="@start" />')</xsl:with-param>
