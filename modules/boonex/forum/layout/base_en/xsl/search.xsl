...
<xsl:template match="search">
    ...
    <xsl:variable name="menu_links">		
        ...
        <btn href="javascript:void(0);" onclick="return f.showSearch()" icon="">New Search</btn>
    </xsl:variable>

    <xsl:call-template name="box">
        <xsl:with-param name="title">Search Results For: '<xsl:value-of select="search_text" />'</xsl:with-param>
        <xsl:with-param name="content">

            <xsl:if test="0 = count(sr)">
                <div style="text-align:center;" class="bx-def-font-large bx-def-bc-padding">
                    There are no search results. <br />
                    Please <a href="javascript:void(0);" onclick="return f.showSearch()" >try again</a>.
                </div>
            </xsl:if>
            <xsl:if test="0 != count(sr)">
...
<xsl:template match="sr">
	...
	<tr>
		...
		<td style="width:15%;" class="forum_table_column_others forum_search_cell_author" valign="top">
            ...
            <xsl:if test="string-length(u/avatar) &gt; 0">
