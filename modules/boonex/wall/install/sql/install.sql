...
SELECT @iPCOrderIndex:=MAX(`Order`) FROM `sys_page_compose` WHERE `Page`='index' AND `Column`='1';
SELECT @iPCOrderProfile:=MAX(`Order`) FROM `sys_page_compose` WHERE `Page`='profile' AND `Column`='2';
SELECT @iPCOrderMember:=MAX(`Order`) FROM `sys_page_compose` WHERE `Page`='member' AND `Column`='1';
INSERT INTO `sys_page_compose` (`Page`, `PageWidth`, `Desc`, `Caption`, `Column`, `Order`, `Func`, `Content`, `DesignBox`, `ColWidth`, `Visible`, `MinWidth`) VALUES
('index', '1140px', 'Events Outline', '_wall_pc_view_index', 1, @iPCOrderIndex+1, 'PHP', 'return BxDolService::call(\'wall\', \'view_block_index\');', 1, 71.9, 'non,memb', 0),
('profile', '1140px', 'Post event to a Timeline', '_wall_pc_post', 2, @iPCOrderProfile+1, 'PHP', 'return BxDolService::call(\'wall\', \'post_block\', array($this->oProfileGen->_iProfileID));', 1, 71.9, 'non,memb', 0),
('profile', '1140px', 'View events on a Timeline', '_wall_pc_view', 2, @iPCOrderProfile+2, 'PHP', 'return BxDolService::call(\'wall\', \'view_block\', array($this->oProfileGen->_iProfileID));', 1, 71.9, 'non,memb', 0),
('member', '1140px', 'View events on a Timeline', '_wall_pc_view_account', 1, @iPCOrderMember+1, 'PHP', 'return BxDolService::call(\'wall\', \'view_block_account\', array($this->iMember));', 1, 71.9, 'memb', 0),
('wall', '1140px', 'Post event to a Timeline', '_wall_pc_post', 1, 0, 'Post', '', 1, 100, 'non,memb', 0),
...
(2, 'bx_wall', '_wall_admin_menu_sitem', '{siteUrl}modules/?r=wall/admin/', 'For managing Timeline', 'time', '', '', @iOrder+1);
...
('wall_events_per_page_profile', '10', @iCategoryId, 'Number of events are displayed on Profile page', 'digit', '', '', 3, ''),
('wall_events_per_page_account', '20', @iCategoryId, 'Number of events are displayed on Account page', 'digit', '', '', 4, ''),
('wall_events_per_page_index', '20', @iCategoryId, 'Number of events are displayed on Home page', 'digit', '', '', 5, ''),
('wall_rss_length', '5', @iCategoryId, 'The length of RSS feed', 'digit', '', '', 6, ''),
('wall_events_hide_timeline', '', @iCategoryId, 'Hide events from Timeline', 'select_multiple', '', '', 7, 'PHP:return BxDolService::call(\'wall\', \'get_actions_checklist\', array(\'timeline\'));'),
('wall_events_hide_outline', '', @iCategoryId, 'Hide events from Outline', 'select_multiple', '', '', 7, 'PHP:return BxDolService::call(\'wall\', \'get_actions_checklist\', array(\'outline\'));');
