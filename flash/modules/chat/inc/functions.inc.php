...
function doRoom($sSwitch, $sUserId = "", $iRoomId = 0, $sTitle = "", $sPassword = "", $sDesc = "", $bTemp = false)
{
    ...
    switch ($sSwitch) {
        ...
        case 'delete':
            $sSql = useServer()
                ? "DELETE FROM `" . MODULE_DB_PREFIX . "Rooms` WHERE `ID` = '" . $iRoomId . "' LIMIT 1"
                : "UPDATE `" . MODULE_DB_PREFIX . "Rooms` SET `When`='" . $iCurrentTime . "', `Status`='" . ROOM_STATUS_DELETE . "' WHERE `ID` = '" . $iRoomId . "'";
            getResult($sSql);
            break;
