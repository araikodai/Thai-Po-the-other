...
class BxDolForm
{
    ...
    {
        if($GLOBALS['MySQL']->getParam('sys_security_form_token_enable') != 'on')
            return;
...
class BxDolFormChecker
{
    ...
    function check (&$aInputs)
    {
        ...
        if($GLOBALS['MySQL']->getParam('sys_security_form_token_enable') == 'on' && $this->_bFormCsrfChecking === true && ($mixedCsrfTokenSys = BxDolForm::getCsrfToken()) !== false) {
